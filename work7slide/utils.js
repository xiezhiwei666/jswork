...
function bubbleSort(str) {
    ...
    for (let i = 0; i<arr.length - 1; i++) {
        ...
        for (let j = 0; j < arr.length - i - 1; j++) {
            //把比较的数组状态和定位指针保存到sortLog中，为保持演示效果，加多两行。
...
function showLog(logValues,showElement){
    ...
    //以形成动画效果。
    if (pos[0] != -1)
    setTimeout("showLog(logValues,showDiv)",700)
}
//插入排序
...
/*function showLog(logValues,showElement) {
    ...
    showElement.innerHTML = str
    if(pos[0] !=-1)
        setTimeout("showLog(logValues,showDiv)",900)
}*/